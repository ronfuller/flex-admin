...
it('should filter the query by a date range')
    ->expect(fn () => Flex::for(Property::class, FIELD::CONTEXT_INDEX)->withoutDefaultFilters()->query(createRequest(['filter' => 'created_at:Last 7 days']))->resource)
    ->toHaveCount(2)
    ->group('filter');

it('should filter the query by last 30 days')
    ->expect(fn () => Flex::for(Property::class, FIELD::CONTEXT_INDEX)->withoutDefaultFilters()->query(createRequest(['filter' => 'created_at:Last 30 days']))->resource)
    ->toHaveCount(4)
    ->group('filter');
