...
it('should parse filter values')
    ->expect(fn () => (new FlexFilterWrapper())->wrapParseFilter(['filter' => buildFilter('company:1;property:2')]))
    ->toBe(['company' => 1, 'property' => 2])
    ->group('filter');

it('should parse filter values with floats, spaces, dashes')
    ->expect(fn () => (new FlexFilterWrapper())->wrapParseFilter(['filter' => buildFilter('company:1.0;property:2;type:  this ; some_thing: else-or-else')]))
...
it('should get filters from attributes')
    ->expect(fn () => (new FlexFilterWrapper())->wrapGetFilters(['filter' => buildFilter('company%3A123;type:small')]))
